...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, in_features, num_heads):
        ...
        self.num_heads = num_heads
        self.attention_heads = nn.ModuleList([AttentionLayer(in_features) for _ in range(num_heads)])
        self.output_linear = nn.Linear(in_features * num_heads, in_features)

    def forward(self, x_self, x_neigh, x_rela):
        head_outputs = [head(x_self, x_neigh, x_rela) for head in self.attention_heads]
