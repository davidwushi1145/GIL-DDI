"""
GIL-DDI Task 2: Prediction of interaction events between known drugs and new drugs

This script trains and evaluates the GNN model for predicting drug-drug interactions
involving new drugs not seen during training. This addresses the feature-shift problem
...
from model.task2.gnn_model import GNN1, GNN2, GNN3
from model.task2.layers import FusionLayer
...
parser = argparse.ArgumentParser(description='GNN based on task2')
...
parser.add_argument("--lr", type=float, choices=[1e-3, 1e-4, 1e-5, 4 * 1e-3], default=1e-3,
                    help="Learning rate")
parser.add_argument("--neighbor_sample_size", choices=[4, 6, 10, 16], type=int, default=8,
...
def train(train_x, train_y, test_x, test_y, net, invariant_adj, early_stopping):
    ...
    train_x_total = torch.cat([train_x1, train_x], dim=0).to(device)
    train_y = torch.cat([train_y, train_y], dim=0).to(device)
    ...
    for epoch in range(args.epoches):
        ...
        with torch.no_grad():
            ...
            if torch.isnan(test_output).any() or torch.isinf(test_output).any():
                print("Nan or Inf in test_output")
...
def main():
    ...
    for cross_ver in range(10):
        ...
        for i in range(len(drugA)):
            ...
            if (drugA_id[i] not in np.array(train_drug[cross_ver])) and (
                    drugB_id[i] in np.array(train_drug[cross_ver])):
                X_test.append(i)
                y_test.append(i)
            if (drugA_id[i] in np.array(train_drug[cross_ver])) and (
    ...
    each_headers = ["Accuracy", "ROC_AUPR", "Placeholder1", "F1_Binary", "Precision_Binary",
                    "Recall_Binary"]
    save_result("../result/", "all", result_all, 2, headers=all_headers)
    save_result("../result/", "each", result_eve, 2, headers=each_headers)
